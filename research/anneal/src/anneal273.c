...
static int best_residue(int m, int *gain, double noise){
  int nt=0;
  for(int i=0;i<un;i++){ int c = ulist[i]%m; if(zc[c]==0) touched[nt++]=c; zc[c]++; }
...
int main(int argc,char**argv){
  ...
  if(!cnt||!ulist||!upos||!zc||!touched){ fprintf(stderr,"oom\n"); return 3; }
  ...
  FILE*L=fopen(logf,"w");
  if(!L) L=stderr;
  fprintf(L,"# anneal273 N=%d K=%d S=%.6f seed=%lld secs=%.0f noise=%.2f fix3=%d\n",N,K,S,seed,secs,noise,fix3);
